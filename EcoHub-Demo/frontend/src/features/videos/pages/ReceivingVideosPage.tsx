...
import { Video, Search, Upload, Download, Eye, Save } from 'lucide-react';
...
export default function ReceivingVideosPage() {
  ...
  const canEditNotes = user?.roles?.some((r) => ['super_admin', 'admin', 'customer_service', 'staff'].includes(r)) ?? false;
  ...
  const [editForm, setEditForm] = useState({
    status: 'pending' as 'pending' | 'matched' | 'mismatched' | 'disputed',
    notes: '',
  });

  const getPlayableUrl = (url?: string | null): string => {
    if (!url) return '';
    const sep = url.includes('?') ? '&' : '?';
    return accessToken ? `${url}${sep}access_token=${accessToken}` : url;
  };

  const getDownloadUrl = (url?: string | null): string => {
    if (!url) return '';
    const sep = url.includes('?') ? '&' : '?';
  ...
  const handleOpenView = (video: ReceivingVideo) => {
    setSelectedVideo(video);
    const cleanNotes = video.comparisonNotes ? video.comparisonNotes.split('\n\n--- Lịch sử chỉnh sửa ---')[0] : '';
    setEditForm({
      status: video.comparisonStatus,
      notes: cleanNotes,
    });
  ...
  const updateNotesMutation = useMutation({
    mutationFn: (data: { id: string; status: string; notes: string }) =>
      videosApi.updateReceivingVideo(data.id, {
        comparisonStatus: data.status as any,
        comparisonNotes: data.notes,
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['receiving-videos'] });
      toast.success('Đã cập nhật đối soát video hoàn hàng');
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal
        ...
        title={selectedVideo ? `Video hoàn hàng - ${selectedVideo.trackingCode}` : 'Xem video'}
        size="xl"
      >
        {selectedVideo?.videoUrl ? (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div className="lg:col-span-7 space-y-4">
              ...
              />
              <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-gray-600 bg-gray-50 p-3 rounded-lg">
                <span className="font-mono break-all text-xs flex-1 mr-2">{selectedVideo.videoUrl}</span>
                <a
                  href={getDownloadUrl(selectedVideo.videoUrl)}
                  download
            ...
            <div className="lg:col-span-5 flex flex-col justify-between space-y-4">
              {canEditNotes ? (
                <form onSubmit={handleSaveNotes} className="space-y-4 flex-1 flex flex-col justify-between">
                  <div className="space-y-4 flex-1">
                    <div>
                      <label className="mb-1 block text-sm font-medium text-gray-700">Trạng thái đối soát</label>
                      <select
                        className="input"
                    ...
                    <div>
                      <label className="mb-1 block text-sm font-medium text-gray-700">Ghi chú khiếu nại</label>
                      <textarea
                        className="input min-h-[80px]"
                        value={editForm.notes}
                        onChange={(e) => setEditForm({ ...editForm, notes: e.target.value })}
                        placeholder="Nhập ghi chú hoặc lý do đối soát..."
                        rows={3}
                      />
                    </div>

                    {(() => {
                      const oldNotes = selectedVideo.comparisonNotes || '';
                      const historyHeader = '\n\n--- Lịch sử chỉnh sửa ---';
                      const history = oldNotes.includes(historyHeader) 
                        ? oldNotes.substring(oldNotes.indexOf(historyHeader) + historyHeader.length) 
                        : '';
                      if (!history) return null;
                      return (
                        <div className="rounded-lg bg-gray-50 p-3 border border-gray-100">
                          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Lịch sử chỉnh sửa</p>
                          <pre className="font-sans text-xs text-gray-600 whitespace-pre-wrap leading-relaxed max-h-40 overflow-y-auto">
                            {history.trim()}
                          </pre>
                        </div>
                      );
                ...
                <div className="space-y-4 flex-1 flex flex-col justify-between">
                  <div className="space-y-4 flex-1">
                    <div>
                      <span className="text-sm font-medium text-gray-500">Trạng thái đối soát: </span>
                      <span
                        className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${
                          comparisonStatusMap[selectedVideo.comparisonStatus].className
                        }`}
                      >
                        {comparisonStatusMap[selectedVideo.comparisonStatus].label}
                      </span>
                    </div>

                    <div>
                      <p className="text-sm font-medium text-gray-500">Ghi chú khiếu nại:</p>
                      <p className="text-sm text-gray-950 mt-1 bg-gray-50 p-3 rounded-lg italic">
                        {selectedVideo.comparisonNotes ? selectedVideo.comparisonNotes.split('\n\n--- Lịch sử chỉnh sửa ---')[0] : 'Không có ghi chú.'}
                      </p>
                    </div>
