...
const syncSessionToAgent = async (session: captureSessionService.CaptureUploadSession) => {
  const [flowResult, orderResult] = await Promise.all([
    captureService.forwardPost('/api/recording_flow', { recording_flow: session.recordingFlow }),
...
export const manualScan = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await captureRuntimeService.processManualScan(req.user!.userId, String(req.body?.code || ''));
    success(res, result);
  } catch (error) {
    next(error);
  }
