...
const canAccessShop = async (currentUser: CurrentUser, shopId: string | null) => {
  if (!shopId || canManageAllOrders(currentUser.roles || [])) return true;
...
export const prepareUploadSession = async (
  ...
  if (!(await canAccessShop(currentUser, order.shopId))) {
  ...
  const module = recordingFlow === 'return' ? 'receiving' : 'packaging';

  if (recordingFlow !== 'return' && ['cancelled', 'returned'].includes(order.status)) {
    throw badRequest(
      'Đơn hàng đã hủy hoặc đã hoàn, không thể quay video đóng gói'
    );
  }
  ...
  const session: CaptureUploadSession = {
    ...
    recordingFlow,
    uploadEndpoint:
      module === 'receiving' ? '/api/videos/receiving/upload' : '/api/videos/upload',
...
export const ensureActiveRecordingSession = async (
  ...
  const resolvedModule = runtime.recording_flow === 'return' ? 'receiving' : module;
