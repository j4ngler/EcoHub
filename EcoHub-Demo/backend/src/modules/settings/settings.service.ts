...
export const getS3Capacity = async () => {
  const shipperEmail = 'shipper-public@ecohub.local';
  const shipperUser = await prisma.user.findUnique({
  ...
  const receivingVideoCustomerAggregate = await prisma.receivingVideo.aggregate({
    where: {
      deletedAt: null,
      customerId: { not: shipperUserId },
    },
    _sum: {
      videoSize: true,
    },
  ...
  const receivingVideoShipperAggregate = await prisma.receivingVideo.aggregate({
    where: {
      deletedAt: null,
      customerId: shipperUserId,
    },
    _sum: {
      videoSize: true,
    },
  ...
  const genericVideoPackaging = await prisma.video.aggregate({
    where: {
      status: 'READY',
      module: 'packaging',
    },
  ...
  const genericVideoCustomerReceiving = await prisma.video.aggregate({
    where: {
      status: 'READY',
      module: 'receiving',
      uploaderUserId: { not: shipperUserId },
    },
    _sum: {
      sizeBytes: true,
    },
  ...
  const genericVideoShipperReceiving = await prisma.video.aggregate({
    where: {
      status: 'READY',
      module: 'receiving',
      uploaderUserId: shipperUserId,
    },
    _sum: {
      sizeBytes: true,
    },
  ...
  const packageVideoSize = packageVideos.reduce((total, video) => {
  ...
  const packagingSize = packageVideoSize + Number(genericVideoPackaging._sum.sizeBytes || 0);

  const returnsSize =
    Number(receivingVideoCustomerAggregate._sum.videoSize || 0) +
    Number(genericVideoCustomerReceiving._sum.sizeBytes || 0);

  const shipperSize =
    Number(receivingVideoShipperAggregate._sum.videoSize || 0) +
    Number(genericVideoShipperReceiving._sum.sizeBytes || 0);

  const packagingCount = packageVideos.length + (genericVideoPackaging._count || 0);
  const returnsCount = (receivingVideoCustomerAggregate._count || 0) + (genericVideoCustomerReceiving._count || 0);
  const shipperCount = (receivingVideoShipperAggregate._count || 0) + (genericVideoShipperReceiving._count || 0);

  const totalSize = packagingSize + returnsSize + shipperSize;
  const totalCount = packagingCount + returnsCount + shipperCount;
  ...
  return {
    ...
    modules: {
      packaging: {
        sizeBytes: packagingSize,
        count: packagingCount,
      },
      receiving: {
        sizeBytes: returnsSize,
        count: returnsCount,
      },
      shipper: {
        sizeBytes: shipperSize,
        count: shipperCount,
      },
    },
  };
