...
export const createUser = async (data: CreateUserDto, createdBy: string, currentUser?: CurrentUser) => {
  // Lấy role để kiểm tra xem có phải vai trò không bắt buộc shop không (super_admin, admin)
  let isOptionalShopRole = false;
  if (data.roleId) {
    const role = await prisma.role.findUnique({ where: { id: data.roleId } });
    if (role && (role.name === RoleName.super_admin || role.name === RoleName.admin)) {
      isOptionalShopRole = true;
    }
  }
  ...
  if (!isOptionalShopRole && !data.shopId) {
  ...
  if (data.roleId) {
    await prisma.userRole.create({
      data: {
        userId: user.id,
        roleId: data.roleId,
        shopId: isOptionalShopRole && !data.shopId ? null : data.shopId,
